...
if __name__ == '__main__':
    ...
    # or desired return
    TARGET_RETURN = None #0.02685 #0.021 #0.021
    ###
    ...
    # starting month of backtest
    initialMonth = 6
    # number of months of total backtest
    numberOfMonths = 2
    # smaller moving windows, only used if COMPLETE_BACKTESTING = True
    movingWindowTesting = 1
    startDateObj = CURRENT_PORTFOLIO and (datetime.today()-timedelta(days=2)) or datetime(year=2017,month=initialMonth,day=11)
    bestParamsDays = None
    bestAveGlobalSharpe = 0.0
    for periodDaysTest in [7]:#,2,3,4,5,6,7]:#range(5,21,5): #range(75,76,1):#range(1,181):#[60]:#[2**i for i in range(0,7)]:
        #if periodDaysTest > 60:
        #    periodDaysTest = 60
        for empiricTraining in [True]:
            for trainingDaysTest in [6]:#[6]:#[240]:#[5,6,7]:
                totalReturn = 1.0
                aveGlobalSharpe = 0.0
                count = 0
                ...
                for i in range(numberOfTests):#periodDaysTest):
                    ...
                    if ret and bestRet:
                        ...
                        try:
                            ...
                            count += 1
                            log.write('Tdays=%d Empiric=%s start%s end%s EstRet=%.2f EstSigma=%.2f EstSharpe=%.2f Ret=%.2f Sigma=%.2f Sharpe=%.2f MaxDD=%.2f AccumRet=%.2f AcumRetLeve=%.2f\n' % 
